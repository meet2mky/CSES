...
public:
    SCC(int n)
    {
        ...
        this->done = false;
        this->compnum = vector<int>(this->n);
    }
    ...
    void findscc()
    {
        ...
        int root = -1;
        for (int i = 0; i < (int)compo.size(); i++)
        {

            for (auto node : compo[i])
            {
                compnum[node] = i;
            }
            if (compo[i].size() >= 2)
    ...
    void dfs(int root, VB &vis, vector<int> &par)
    {
        if (SZ(anycycle) != 0)
            return;
        for (auto x : g[root])
        {
            if (!vis[x] && compnum[x] == compnum[root] && SZ(anycycle) == 0)
            {
                par[x] = root;
                vis[x] = true;
                dfs(x, vis, par);
            }
            else if (compnum[x] == compnum[root] && SZ(anycycle) == 0)
            {
                // x is already visited;
                for (int u = root; u != x; u = par[u])
                {
                    anycycle.push_back(u);
                }
                anycycle.push_back(x);
                reverse(ALL(anycycle));
                return;
            }
        }
    }
